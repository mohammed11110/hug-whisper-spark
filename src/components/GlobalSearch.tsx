    ...
    const handle = setTimeout(async () => {
      ...
      (us || []).forEach((u: any) => {
        if (u.tenant_name) {
          ...
          results.push({ type: "unit", id: u.id, title: `${allBMap.get(u.building_id) || ""} · ${u.unit_number}`, subtitle: "وحدة", to: `/units/${u.id}` });
        }
      });
      (ps || []).forEach((p: any) => results.push({
        type: "payment", id: p.id, title: `إيصال #${p.receipt_number}`, subtitle: `${p.amount} · ${p.payment_date}`, to: `/payments`,
      }));
      setItems(results);
      setLoading(false);
    }, 250);
      ...
      <div className="max-w-[430px] mx-auto pt-16 px-4" onClick={(e) => e.stopPropagation()}>
        <div className="bg-card rounded-2xl shadow-glow border border-sage-200/40 overflow-hidden">
          <div className="flex items-center gap-2 p-3 border-b border-sage-200/40">
            ...
            <Input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="ابحث: مبنى، مستأجر، إيصال..."
              className="border-0 focus-visible:ring-0 h-9 px-0" />
            <button onClick={onClose} className="text-muted-foreground hover:text-foreground"><X className="h-4 w-4" /></button>
          </div>
          <div className="max-h-[60vh] overflow-y-auto">
            {loading && <p className="text-center text-xs text-muted-foreground py-6">جاري البحث...</p>}
            {!loading && q && items.length === 0 && <p className="text-center text-xs text-muted-foreground py-6">لا توجد نتائج</p>}
            {!loading && !q && <p className="text-center text-xs text-muted-foreground py-6">اكتب للبحث في كل بياناتك</p>}
            {items.map((it) => (
