  ...
  const send = async (textOverride?: string) => {
    ...
    try {
      ...
      const { data: us } = bIds.length
        ? await supabase.from("units").select("id, building_id, status, rent_amount, tenant_name, contract_end_date").in("building_id", bIds)
        : { data: [] as any[] };
      ...
      const { data: ps } = unitIds.length
        ? await supabase.from("payments").select("amount, payment_date, unit_id").in("unit_id", unitIds).gte("payment_date", start).is("deleted_at", null)
        : { data: [] as any[] };
