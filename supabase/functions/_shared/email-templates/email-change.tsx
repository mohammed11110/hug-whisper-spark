...
interface Props { oldEmail: string; email: string; newEmail: string; confirmationUrl: string; lang?: string }

export const EmailChangeEmail = ({ oldEmail, newEmail, confirmationUrl, lang }: Props) => {
  const L = normalizeLang(lang)
  const s = getStrings(L, 'email_change')
  const dir = isRtl(L) ? 'rtl' : 'ltr'
  const ff = fontFamily(L)
  const body = typeof s.body === 'function' ? s.body({ oldEmail, newEmail }) : s.body
  return (
    <Html lang={L} dir={dir}>
      <Head />
